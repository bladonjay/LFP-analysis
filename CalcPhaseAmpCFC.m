...
[phasects,~,phaseall] = histcounts(phases,phasebins);
...
% end
phaseAmp=accumarray(phaseall,amps)./phasects'; % the faster way, gup array and then divide
...
if runstat>0
    for j=1:runstat
        ampsB=circshift(amps,randi(length(amps)-guardrail*2)+guardrail);
